...
```bash
# 跑前 lies 数据
$env:ONLY="creepjs"; pnpm --filter @runova/sdk exec tsx bench/baseline-detection.ts
...
**验证**：重跑 `pnpm --filter @runova/sdk run bench:all`，期望：
...
```bash
pnpm --filter @runova/sdk bench:canvas
# 或
...
```bash
pnpm --filter @runova/sdk exec tsx bench/baseline-detection.ts                 # 全部 9 站
ONLY=dbi-bot,amiunique,pixelscan pnpm --filter @runova/sdk exec tsx bench/baseline-detection.ts
HEADED=1 ONLY=pixelscan pnpm --filter @runova/sdk exec tsx bench/baseline-detection.ts  # pixelscan 推荐 headed
...
- 等本地网络恢复后跑：
  ```bash
  $env:ONLY='sannysoft,dbi-bot'; pnpm --filter @runova/sdk run bench:all; Remove-Item env:ONLY
  ```
...
# 重跑 baseline（验证起点状态）
pnpm --filter @runova/sdk run bench:all
```
