...
export function renderGeneratedSource(profiles: readonly IntegratedProfile[]): string {
  ...
  const header = `${headerComment}import type { GlParamValue, WebglProfile } from './webgl-profiles.js';\n\n`;
  ...
  const registry = `export const KNOWN_PROFILES_CAPTURED: readonly WebglProfile[] = [\n${sorted.map((p) => `  ${p.constName},`).join('\n')}\n];\n`;
