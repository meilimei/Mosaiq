...
function main() {
  ...
  console.log(
    `  brandHash:       ${baseline.brandHash} (in whitelist: ${baseline.brandInWhitelist})`,
  );

  const baselineNearest = nearestCapHash(baseline.capHash);
  console.log(
    `  nearest cap whitelist hash: ${baselineNearest.value} (diff ${baselineNearest.diff})`,
  );
  ...
  if (hits.length === 0) {
    ...
    console.log(
      `     expected joint hit: 1 / ${(1 / (capDensity * brandDensity)).toExponential(2)} tries`,
    );
    ...
    console.log(
      '   - Whitelist is curated from real-user submissions, not algorithmically derived',
    );
