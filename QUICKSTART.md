...
import { loadPersona, launchPersona, runDetection } from '@runova/sdk';
...
--patch-dir node_modules/@runova/sdk/patches` 显式 apply，或者放开 postinstall。
...
pnpm --filter @runova/sdk demo:humanize

# Phase 7 captured-WebGL-profiles 工作流
pnpm --filter @runova/sdk run bench:integrate-profiles            # 从 bench/captured-profiles/*.json 重新生成 TS
pnpm --filter @runova/sdk run bench:integrate-profiles -- --check # CI drift gate（exit 1 = 漂移）
...
- 临时设置代理：`$env:HTTPS_PROXY="http://127.0.0.1:7890"; pnpm --filter @runova/sdk exec playwright install chromium`
...
CLI 没有自己的反检测/注入逻辑 — 它是 `@runova/sdk` 公共 API（`runDetection` / `formatDetectionRunMarkdown` / `diffRuns` / `listPersonas` / `savePersona` 等）的 thin wrapper。所有"算 detection score"、"format markdown"、"diff runs" 这种 pure logic 都住在 SDK，CLI 只做 argv 解析 + TTY 渲染 + 退出码策略。这意味着任何第三方脚本只要 `import { runDetection } from '@runova/sdk'` 就能复制 CLI 行为。
...
- **patch-package postinstall 与 `--ignore-scripts` 不兼容**（CI 用 `npm ci --ignore-scripts` 时反检测降级）
  → 文档明确写了绕开方法（手工 `npx patch-package --patch-dir node_modules/@runova/sdk/patches`）。runtime warn 检查推到 v0.11 评估
...
2. **`pnpm --filter @runova/sdk demo:humanize`**，DevTools Performance 录鼠标轨迹确认是曲线
