...
- ✅ `@mosaiq/sdk` 的 `patches/playwright-core@1.59.1.patch` 在
...
> ⚠️ 顺序敏感: `@mosaiq/sdk` 依赖 `@mosaiq/persona-schema`，
...
```bash
npm view @mosaiq/persona-schema version    # → 0.10.0
npm view @mosaiq/sdk version               # → 0.10.0
...
npx playwright install chromium            # @mosaiq/sdk 依赖 playwright-core
...
npm unpublish @mosaiq/sdk@0.10.0 --force
npm unpublish @mosaiq/persona-schema@0.10.0 --force
```
...
- 三包上 npm
- 自动打 git tag `@mosaiq/persona-schema@X.Y.Z` 等（**注意：不是
  `vX.Y.Z`**）
...
npm unpublish @mosaiq/sdk@X.Y.Z --force
npm unpublish @mosaiq/persona-schema@X.Y.Z --force
...
npm deprecate @mosaiq/sdk@X.Y.Z "Critical bug, use X.Y.(Z+1) or later"
npm deprecate @mosaiq/persona-schema@X.Y.Z "Critical bug, use X.Y.(Z+1) or later"
```
...
- **patches/playwright-core@1.59.1.patch 缺失**: external smoke 时
  `npm i @mosaiq/sdk` 没自动 apply patch → 检查 `.npmignore` 是否把
