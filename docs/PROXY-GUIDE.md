...
| 服务商 | username 格式 | session 时长控制 |
|---|---|---|
| IPRoyal | `<user>_country-us_session-<random8>_lifetime-30m` | 后缀 `lifetime-Xm` |
| Decodo (Smartproxy) | `user-<user>-session-<random>-sessionduration-30` | 后缀 `sessionduration-X` |
| BrightData | `<user>-session-<random>` | 默认 1-30 分钟 |
| Soax | `<package_id>-session-<random>` | 5-30 分钟 |

**Mosaiq 集成**：用户名字段直接填带后缀的完整字符串，Mosaiq 会原样传给代理服务。`label` 字段建议写成 `iproyal-us-aliceabc-30m` 便于审计。
...
- **池规模**：32M+ IPs，195 国
- **endpoint**：`residential.iproyal.com:12321`
- **计价**：纯按量（GB）
  - 默认 $7/GB
  - 50GB+ $4.5/GB
  - 500GB+ $1.75/GB
- **起付**：$8（≈ 1GB），充值制
- **Sticky**：用户名后缀，`lifetime-1m` 到 `lifetime-30m`
- **优点**：
  - 个人友好，起付最低
  - 充值不限期不过期
  - Dashboard 简洁，KYC 宽松
...
5. Dashboard 显示：
   - **Hostname**: `residential.iproyal.com`
   - **Port**: `12321`
   - **Username**: `<your_username>`（形如 `abc123def456`）
   - **Password**: `<your_password>`

### 4.2 sticky session 命名约定

每个 persona 一个独立 session ID。**绝对不能**多个 persona 共用同一 username 后缀（= 同 IP = 关联）。

```
模板：<base_username>_country-<cc>_session-<8位随机>_lifetime-<分钟>m

示例：
reddit-alice  → abc123def456_country-us_session-aliceR4nd_lifetime-30m
reddit-bob    → abc123def456_country-us_session-bobX9z2k_lifetime-30m
reddit-cara   → abc123def456_country-us_session-caraQw7p_lifetime-30m
```
...
|---|---|
| 协议 | HTTP |
| 标签 | `iproyal-us-aliceR4nd-30m` |
| 主机 | `residential.iproyal.com` |
| 端口 | `12321` |
| 用户名 | `<base_username>_country-us_session-aliceR4nd_lifetime-30m` |
| 密码 | `<your_password>` |
