...
| 实现工作量 | 4 commits + 1 hotfix（4c），2-3 天 |
...
### Commit 4b: `feat(cloud-runtime): stagehand-compat-smoke.mjs — real Browserbase SDK smoke test` — LANDED
...
- `package.json`：新增 root devDeps `@browserbasehq/sdk ^2.6.0`（lockfile pin `2.12.0`） + `playwright-core 1.59.1`（与 repo 现有 pin 一致，复用同一 patch）；npm 别名 `smoke:stagehand-compat`。
- 本地验证：`biome check` 绿 + `node --check` 绿。
- Prod 实跑：见 §6.1，5 次连跑全 pass（n=15 sessions），mean acquire 35.5s，0 retry。

**验收**（已落地）：smoke 在 prod 跑通，5 次连续运行全 pass，每次 acquire latency < 60s（pool warm）。

### Commit 4c: `fix(cloud-runtime): per-session signing key embedded in connectUrl ?token=` — LANDED

首次 prod smoke run（commit 4b 落地后立刻起跑）暴露根因：`session.connectUrl` 形如 `wss://...fly.dev/v1/sessions/:id/cdp`，**不带任何凭据**。Stagehand 调 `chromium.connectOverCDP(session.connectUrl)` 时 Playwright 默认不携 header，控制平面 WS upgrade auth 路径里 Bearer header 和 `?token=` 都拿不到 → 401 → smoke s1_empty fatal。

**根因**：phase 11.4a `shapeSession()` 把 BB 兼容字段 `signingKey: null` 当作 stub 输出（理由是 "phase 11.4a 不实现"），但 BB 自家行为是 `signingKey` 非 null 且**已被嵌入** `connectUrl?token=...`。Stagehand 依赖该机制——这是漏掉的强语义契约，不是可选字段。

**修复**（最小 upstream fix，全在 cloud-runtime 内）：
- `db/schema.ts`：`sessions` 加 nullable `signing_key` 列（`sks_<22>` 前缀 ID）。
- `db/bootstrap.ts`：CREATE TABLE 加列 + COLUMN_ADDITIONS 加 `ALTER TABLE` 让 prod 老 DB 平滑升级。
- `utils/ids.ts`：`IdPrefix` 加 `'sks'`。
- `routes/sessions.ts`：每个 new session create 时 `newId('sks')` 并落库；`shapeSession()` 把 `?token=<signingKey>` 内嵌进 `cdp_url` + `connectUrl`，`signingKey` 字段返回真实值（不再 null）。
- `cdp/proxy.ts`：WS upgrade auth 改三叉 fallback：(1) Bearer header → api_key；(2) `?token=` 与 row.signingKey 严等 → session_key；(3) `?token=` 当 api_key plaintext。session_key 路径仅授权该 session（与 api_key 全 project scope 互斥），用 `crypto.timingSafeEqual` 防 timing leak。
- 测试：`app.test.ts` 移除 "signingKey 为 null" 断言，新增 "signingKey 非空且与 connectUrl `?token=` 完全一致" 断言；GET 路径同样验证 echo。`pnpm typecheck` 绿，`pnpm test` **186/186 全绿**（+4 vs commit 4b）。

**验收**（已落地）：fly v11→v12 redeploy（image `01KSK9NYJKX99Z6KQHS55GG57F`），随后 §6.1 5 次连跑、15/15 sessions 全 pass。
...
实测于 prod 部署 `deployment-01KSK9NYJKX99Z6KQHS55GG57F`（2026-05-26 23:27 UTC，commit `d061ef5` =
phase 11.4 commit 4c "per-session signing key embedded in connectUrl `?token=`" 落地后）。
Pool 状态 `ready=9 busy=1 cap=10`（POOL_TARGET_SIZE=5 secret，replenish_concurrency=2）。

每次 smoke 跑三个递增覆盖的 BB-SDK 调用：
- **s1**：`bb.sessions.create({})` —— 默认 persona seed + X-BB-API-Key + connectUrl 接通
- **s2**：`bb.sessions.create({ userMetadata: {...} })` —— BB request body 接受 + userMetadata 落库 + 回显
- **s3**：`bb.sessions.create({ browserSettings: { viewport } })` —— browserSettings.viewport honor

每个场景闭环跑 `goto https://example.com → 验 title === "Example Domain" → DELETE`。

| 跑次 | 时间 (UTC) | acquire ms (s1/s2/s3) | connect ms (s1/s2/s3) | 结果 | 备注 |
|---|---|---|---|---|---|
| #1 | 23:29:24 | 33884 / 37082 / 33792 | 7595 / 4192 / 7760 | ✓ all_done | wall=141.6s, 0 retries |
| #2 | 23:32:01 | 33870 / 37016 / 33815 | 7576 / 4291 / 7483 | ✓ all_done | wall=141.3s, 0 retries |
| #3 | 23:34:38 | 34401 / 37178 / 34248 | 7798 / 4232 / 7610 | ✓ all_done | wall=141.5s, 0 retries |
| #4 | 23:37:23 | 39008 / 34771 / 37209 | 4221 / 7699 / 4115 | ✓ all_done | wall=143.6s, 0 retries |
| #5 | 23:40:03 | 37112 / 34440 / 34352 | 4334 / 7707 / 7598 | ✓ all_done | wall=144.2s, 0 retries |

**汇总（n=15 sessions across 5 runs）**：

| 指标 | min | mean | max | 备注 |
|---|---|---|---|---|
| acquire ms | 33792 | 35,492 | 39008 | `bb.sessions.create()` HTTP 往返 + `mm.acquire()`（pool consume + 启动 chromium + `/control/start` 握手） |
| connect ms | 4115 | 6,281 | 7798 | Playwright 的 `chromium.connectOverCDP(url)` —— WS upgrade + `Target.attachToBrowserTarget` |
| nav ms | 1879 | ~1981 | 2120 | `page.goto('example.com', waitUntil:'domcontentloaded')` |
| per-scenario total | 45,864 | ~47,055 | 48,785 | s1 + s2 + s3 串行；含 `browser.close()` 和 best-effort DELETE |
| wall (3 scen serial) | 141,342 | 142,445 | 144,157 | 5 跑次 wall 时间 |

**关键观察**：
- **0 失败、0 retry、15/15 sessions 全 pass**：phase 11.4 兼容钩子已可用；Stagehand 改 `baseURL` + `apiKey` 即可迁。
- acquire 35.5s 平均符合 phase 11.3a 文档 §6 的 "warm-pool 35-40s" 预期（pool=5）。phase 11.5 keepAlive 后会进一步降至 < 5s（machine 复用）。
- connect ms 在 4-8s 之间分两簇 —— 其中 ~4.2s 是 chromium 已对外侦听后的 fast path；~7.5s 是首次 attach 时的 sandbox/extensions 初始化。无 retry，两条路径都成功 connect。
- 所有 connectUrl 形如 `wss://...fly.dev/v1/sessions/ses_xxx/cdp?token=sks_yyyy`，`?token=` 即 commit 4c 加入的 per-session signing key；Playwright 不传 header 也接得通。该 fix 在烟测 #1 第一次跑出 401 后即时定位、加测、redeploy（v11→v12），后续 5 次连跑稳定。
- s2 `userMetadata` round-trip 验证：`{source:'stagehand-compat-smoke', runAt:<ISO>}` 全部回显在 `create_ok` 事件 `userMetadata` 字段中。
- s3 `browserSettings.viewport={width:1366, height:768}` 未出现在响应的 `unsupportedFields[]`（即被 honor），其他 BB stub 字段（recording / proxies / extensionId / region / timezone）按设计走 warn-and-ignore 通道。
- 每跑次成本 ~$0.0003-0.001（3 个 shared-2x machine × ~30s + ws 流量 < 1MB）。15 sessions × ~$0.0005 ≈ **$0.008 总成本**。
...
- [x] Commit 4b 落地：scripts/stagehand-compat-smoke.mjs + root devDeps `@browserbasehq/sdk ^2.6.0`（lockfile 2.12.0） & `playwright-core 1.59.1`。biome + node --check 绿。Prod 验证见 §6.1。
- [x] Commit 4c 落地：per-session signing key 内嵌 connectUrl `?token=` + 三叉 WS auth fallback + timing-safe 比较，+4 测试 → **186/186**（修了 commit 4b 第一次 prod smoke 暴露的 401 根因）

### 11.2 prod 验证

- [x] `scripts/stagehand-compat-smoke.mjs` 5 次连跑全 pass（详 §6.1，n=15 sessions, 0 fail / 0 retry，commit 4c v12 部署后）
- [x] 同 5 次跑出的 acquire 进入 `mm_acquire_duration_seconds_count`，让 phase 11.3a §11.3 表 `pool=5 @ post-stagehand` 行有真实数据（mean 35.5s, range 33.8-39.0s, p100=39.0s）
- [-] Stagehand `eval/` 仓库 baseline test —— **PARKED 2026-05-27 → 11.4b 候选**：SDK compat 表面已由 `scripts/stagehand-compat-smoke.mjs` 用真 `@browserbasehq/sdk@2.12.0` + `playwright-core@1.59.1` 验证（详 §6.1，n=15 sessions all-pass），覆盖 `bb.sessions.create()` → `connectUrl` → `chromium.connectOverCDP()` → `page.goto()` 全链路，这正是 Stagehand 内部走的同一段代码。Stagehand 自家 `eval/`（github.com/browserbase/stagehand/evals）测的是 `act()`/`observe()`/`extract()` LLM 驱动方法栈，需要 `OPENAI_API_KEY`/`ANTHROPIC_API_KEY` + 真站点目标，测的是"Stagehand-on-Mosaiq vs Stagehand-on-Browserbase"端到端体验差——属于 §0 TL;DR 列出的"不在 11.4a 范围"的"Stagehand 直发 act/observe 模式"。留作 11.4b（"Stagehand 完整 demo / GTM 公开 leaderboard"）候选内容；当前 11.4a SDK 兼容正确性 gate 不依赖此项。

### 11.3 文档

- [x] 本 doc §6.1 实测结果填表（n=15, 5 runs, mean wall=142.4s）
- [x] `docs/PHASE-11.3-MACHINE-POOL.md` §11.2/§11.3 加补充：pool=1 24h 观测被 phase 11.4a 超越；§11.3 表新增 `pool=5 @ post-stagehand` 行接上 15 个 acquire 样本
- [x] `README.md` 顶部加 Stagehand quickstart code block + 长期愿景章节里"Cloud Runtime alpha 已在 fly.io 跑"声明
