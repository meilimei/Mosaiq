...
- 字段映射：
  ...
  - **必须 fallback**：完全不传任何 persona 时，从 default persona pool 抽一个（phase 11.4 同时落地一个最小 default persona seed，~5 个 fixed personas）
- 测试：
  - BB-shape 空 body `{}` → 创建成功，分配 default persona
  - BB-shape `{"viewport":{"width":1280,"height":720}}` → 应用 viewport
  - BB-shape 未支持字段 → 200 但日志含 warning
  - 同时传 BB-shape + native-shape 字段 → 400（明确 error）

**验收**：156 个测试 + ~10 个新增 BB-input 测试。

### Commit 4: `feat(cloud-runtime): default persona pool seed for parameter-less BB-shape sessions + smoke test`

- `apps/cloud-runtime/src/seed/default-personas.ts`：5 个 hand-curated persona JSON（typical 桌面：Win11 Chrome、macOS Safari、Win11 Edge、Ubuntu Firefox、macOS Chrome）。从 `apps/cloud-runtime/src/seed/data/*.json` 加载。
- `apps/cloud-runtime/src/db/bootstrap.ts`：启动时 if no rows in `personas` table → seed 5 个 default。
- `scripts/stagehand-compat-smoke.mjs`：跑真实 Stagehand SDK：
  ```js
  const bb = new Browserbase({
    apiKey: process.env.MOSAIQ_API_KEY,
    baseURL: process.env.MOSAIQ_BASE_URL,
  });
  const s = await bb.sessions.create({});
  const browser = await chromium.connectOverCDP(s.connectUrl);
  // navigate to https://example.com, check title === "Example Domain"
  ```
...
- [ ] Commit 1 落地：dual auth header，4-5 测试
- [ ] Commit 2 落地：response superset，6 测试
- [ ] Commit 3 落地：BB-shape request body，10 测试
- [ ] Commit 4 落地：default persona seed + stagehand smoke，5 跑次全绿
