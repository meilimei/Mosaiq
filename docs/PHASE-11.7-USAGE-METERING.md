...
- [ ] DELETE / reaper 关闭 session → 恰好一条 `session.minute` usage_event，value 正确
- [ ] acquire 失败 / context-race rollback / keepAlive WS 断 → **不** emit
- [ ] `GET /v1/usage` 跨 project 隔离（只返本 project）+ 时间窗过滤正确
- [ ] report job：noop reporter 下 events 被标 reported；reporter 抛错时保持 NULL 重试
- [ ] 全部既有测试保持绿；迁移在已有 usage_events 表的 prod-like DB 跑通
